for iter = 1:5
    ...
    U22 = UU(:, iter);
    U222 = U22;
    U222reshape = reshape(U222,73,144);
    subplot('Position', [margin, margin, width, height]);
    h1 = heatmap(abs(U222reshape));
    h1.XDisplayLabels = repmat({''}, 1, 144); % 去掉横坐标数字
    h1.YDisplayLabels = repmat({''}, 1, 73); % 去掉纵坐标数字
    ...
    h2.XDisplayLabels = repmat({''}, 1, 144); % 去掉横坐标数字
    h2.YDisplayLabels = repmat({''}, 1, 73); % 去掉纵坐标数字
    ...
    h3.XDisplayLabels = repmat({''}, 1, 144); % 去掉横坐标数字
    h3.YDisplayLabels = repmat({''}, 1, 73); % 去掉纵坐标数字
    ...
    h4.XDisplayLabels = repmat({''}, 1, 144); % 去掉横坐标数字
    h4.YDisplayLabels = repmat({''}, 1, 73); % 去掉纵坐标数字
    h4.GridVisible = 'off'; % 去掉网格
    % title('YetAnotherMethod'); % 添加标题
    saveas(gcf,['figure/NISTSingularVector_T_',num2str(T),'_order_',num2str(iter),'.fig']);
end